...
class DistinctOnBehaviourTestCase(TestCase):
    """
    Our billing change model relies on distinct on working predictably in
    Django.
...
class DistinctOnSubQueryBehaviourTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        with freezegun.freeze_time(str(parse_billing_week('2016-06 1').wed)):
            ...
            # make the customers active
            now = timezone.now()
            bw = get_billing_week(now)
            customer_1_account_status_change = AccountStatusChange(
                changed=now,
                changed_in_billing_week=str(bw),
                customer=cls.customer1,
                status=AccountStatusChange.ACTIVE,
            ).save()
            customer_2_account_status_change = AccountStatusChange(
                changed=now,
                changed_in_billing_week=str(bw),
    ...
    * Line item runner doesn't create entries for:
    ...
    def setUp(self):
        ...
        with freezegun.freeze_time('2015-11-22 11:50', tick=False):
            # Customer signs up with one collection left in month
            now3 = timezone.now()
        ...
        with freezegun.freeze_time('2015-11-22 15:10', tick=False):
            # Customer signs up with no collections left in month
            now4 = timezone.now()
